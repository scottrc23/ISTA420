...
WITH C AS
(SELECT *, DATEFROMPARTS(YEAR(orderdate), 12, 31) AS endofyear FROM Sales.Orders)
...
select o.empid, max(orderdate) as orderdate from sales.orders o group by empid;
...
select o.empid, max (o.orderdate), o.order
select o.empid, max(orderdate) as orderdate from sales.orders o group by empid;

select o.empid, max(o.orderdate), o.orderid, o.custid
from sales.orders o inner join
(select empid, max(orderdate) as dateoflastactivity from sales.orders group by empid) as c
on o.empid = c.empid and o.orderdate = c.dateoflastactivity;
...
as
select o.empid, year(o.orderdate), as orderyear, sum(od.qty) as sumqty
...
create view sales.vemporders
as
select o.empid, year(o.orderdate), od.qty
from sales.orders o inner join sales.orderdetails od
on o.orderid = od.orderid
group by o.empid, year(o.orderdate)
